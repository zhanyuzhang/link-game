...
LinkGame.prototype = {
    ...
    updateDomNumbers: function ($container, value, type) {
        ...
        while(numList.length) {
            $container.append(this.replaceTpl('<img src="${src}" />', {src: nums[numList.pop()]}));
        }
    },
    ...
    drawLine: function (callback) {
        ...
        for(var i = 0; i < linkList.length; i++) {
            var x = linkList[i].col === 0 ? 0 : (linkList[i].col === this.cols - 1 ? $canvas.width() : linkList[i].col * 80 - 40);
            var y = linkList[i].row === 0 ? 0 : (linkList[i].row === this.rows - 1 ? $canvas.height() : linkList[i].row * 80 - 40);
            coordinate.push([x, y]);
        }
